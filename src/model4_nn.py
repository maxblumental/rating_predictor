...
if __name__ == '__main__':
    ...
    holdout_df = pd.read_parquet('../data/holdout.parquet')
    dev_df, holdout_df = preprocess_data(movies_df, dev_df, holdout_df,
                                         min_movies_per_user=10, min_users_per_movie=10)
    params = {
        'model_name': 'rating_predictor',
        'n_epochs': 64,
        'model': {
            'reg': 3e-5,
            'reg_u': 3e-5,
    ...
    train_model(dev_df, holdout_df, params)
    rmse = test_model(dev_df, holdout_df, params)
